...
with st.sidebar:
    ...
    with one: 
        st.image("media/MDST_logo.jpg", width= 200)
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        st.chat_message(msg["role"], avatar=("media/userImg.jpg")).write(msg["content"])
    else:
...
if prompt := st.chat_input("Enter your message here", key = count):
    ...
    st.chat_message("user", avatar=("media/userImg.jpg")).write(prompt)
